class FootballMatchGenerator {
    ...
    attachEventListeners() {
        ...
        this.teamCountInput.addEventListener('input', () => this.validateTeamCount());
        this.teamCountInput.addEventListener('keypress', (e) => {
            if (e.key === 'Enter') this.proceedToTeamNames();
        });

        // Navigation buttons
        this.proceedToTeamsBtn.addEventListener('click', () => this.proceedToTeamNames());
        this.backToCountBtn.addEventListener('click', () => this.goBackToCount());
        this.generateMatchesBtn.addEventListener('click', () => this.generateMatches());
        this.regenerateMatchesBtn.addEventListener('click', () => this.generateMatches());
    ...
    validateTeamCount() {
        ...
        this.teamCountInput.classList.toggle('error', !isValid && this.teamCountInput.value);
    ...
    proceedToTeamNames() {
        if (!this.validateTeamCount()) return;
    ...
    createTeamInputs(count) {
        ...
        for (let i = 1; i <= count; i++) {
            ...
            input.addEventListener('keypress', (e) => {
                if (e.key === 'Enter') this.focusNextInput(i, count);
            });
        }
    ...
    validateTeamName(index) {
        ...
        if (!value) {
            errorMessage = value === '' && input.value !== '' ? 'Team name cannot be empty' : '';
            ...
            const otherInputs = document.querySelectorAll('input[id^="team-"]:not(#team-' + index + ')');
            const duplicateFound = Array.from(otherInputs).some(otherInput => 
